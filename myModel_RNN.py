...
class myModel(nn.Module):
    def __init__(self, userNum, itemNum, time_number, behavior, subgraphs):  #TODO: 这里的参数在调用处要解决
        ...
        self.GRU_layers =  self.init_rnns()       
        self.hidden = None   #这一句能用得上, GRU输出的时候可以
    ...
    def init_rnns(self):
        ...
        for i in range(len(self.behavior)):
            rnn_layers.append(nn.GRU(input_size=args.hidden_dim, hidden_size=args.hidden_dim, batch_first=True))
...
#——————————————————————————————————————————————————————————————————————————————————以下
        ...
        user_behaviors_embedding = {}
        item_behaviors_embedding = {}
        for i in range(len(self.behavior)):
            user_behaviors_embedding[i] = []
            item_behaviors_embedding[i] = []

        #得到RNN
        for i, beh in enumerate(self.behavior):  #记住迭代器不加r
            for t in range(self.time_number):
                user_behaviors_embedding[i].append(self.embedding_dict['times_user_embedding'][t][beh])  #[time_number, ui_num, hidden_dim]
                item_behaviors_embedding[i].append(self.embedding_dict['times_item_embedding'][t][beh])

            user_behaviors_embedding[i] = torch.stack(user_behaviors_embedding[i], dim=0).transpose(0,1)  #这里可能报错： 原来是一个list， 现在是一个tensor所以
            item_behaviors_embedding[i] = torch.stack(item_behaviors_embedding[i], dim=0).transpose(0,1)  #[ui_num, time_number, hidden_dim]  [147894, 9, 8]


        for i, gru_layer in enumerate(self.GRU_layers):   #[ui_num, time_number, hidden_dim].  输入的是一个tensor输出的也是一个tensor.
            user_behaviors_embedding[i], _ = gru_layer(user_behaviors_embedding[i])  
            user_behaviors_embedding[i] = user_behaviors_embedding[i].transpose(0, 1)
            item_behaviors_embedding[i], _ = gru_layer(item_behaviors_embedding[i])
            item_behaviors_embedding[i] = item_behaviors_embedding[i].transpose(0, 1)


        user_embedding_dict = {}
        item_embedding_dict = {}
        for i, beh in enumerate(self.behavior):   #对行为数量个tensor[time_number, ui_num, hidden_dim]
            user_embedding_dict[beh] = user_behaviors_embedding[i][-1]
            item_embedding_dict[beh] = item_behaviors_embedding[i][-1] 
    ...
    #______________________________________________以上
        ...
        user_embedding = self.behavior_attention(user_embedding_dict)
        item_embedding = self.behavior_attention(item_embedding_dict)
    ...
    def behavior_attention(self, embedding_input):  #TODO: query
        ...
        attention = F.softmax(attention, dim=0)*1  #TODO:这里稍微变大一点就loss==nan
