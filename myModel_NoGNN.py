...
class myModel(nn.Module):
    ...
    def init_weight(self):  #TODO: embedding和parameter有什么区别
        ...
        weight_dict = nn.ParameterDict({
            ...
            'w_self_attention_cat': nn.Parameter(initializer(torch.empty([args.hidden_dim*args.head_num, args.hidden_dim]))),
    ...
    def forward(self, subgraphs):
        ...
        for t in range(0, self.time_number):
            ...
            for i, beh in enumerate(self.behavior):
                ...
                self.embedding_dict['times_item_embedding'][t][beh], self.embedding_dict['times_user_embedding'][t][beh] = model.item_embedding.weight, model.user_embedding.weight 
...
#——————————————————————————————————————————————————————————————————————————————————以下
        ...
        for t in range(0, self.time_number):
            if t==0:
                ...
                # user_z = self.self_attention(self.weight_dict, self.embedding_dict['times_user_embedding'][t-1], self.embedding_dict['times_user_embedding'][t]) 
                # item_z = self.self_attention(self.weight_dict, self.embedding_dict['times_item_embedding'][t-1], self.embedding_dict['times_item_embedding'][t])


                for i, beh in enumerate(self.behavior):
                    self.embedding_dict['times_user_embedding'][t][beh] = self.act(torch.matmul((self.weight_dict['alpha'][0]*self.embedding_dict['times_user_embedding'][t][beh] + (1-self.weight_dict['alpha'][0])*user_z[i]), self.weight_dict['w_self_attention_user']))  #1:[N, hidden_dim]  2:[N, hidden_dim]  3:[N, hidden-dim]
                    self.embedding_dict['times_item_embedding'][t][beh] = self.act(torch.matmul((self.weight_dict['alpha'][1]*self.embedding_dict['times_item_embedding'][t][beh] + (1-self.weight_dict['alpha'][0])*item_z[i]), self.weight_dict['w_self_attention_item']))
                    # self.embedding_dict['times_user_embedding'][t][beh] = self.act(torch.matmul((self.embedding_dict['times_user_embedding'][t][beh] + user_z[i])/2, self.weight_dict['w_self_attention_user']))  #1:[N, hidden_dim]  2:[N, hidden_dim]  3:[N, hidden-dim]
    ...
    def self_attention(self, trans_w, embedding_t_1, embedding_t):  #到底要不要写一个类, TODO: 问一下师兄或者大佬, 到底啥时候要写类, 啥时候一个函数就可以解决
        ...
        # print("Z 的大小2:", Z.shape)  #[3,147894, 16]  打印出来就是这个结果, 已经运行出来的

        # tmp_Z = Z
        # cnt = 0
        # judge_is_nan = torch.isnan(tmp_Z.view(-1, 1))
        # for i in judge_is_nan:
        #     if i == True:
        #         cnt+=1
        # print("judge_is_nan_cnt:", cnt)
    ...
    def multi_head_self_attention(self, trans_w, embedding_t_1, embedding_t):  #到底要不要写一个类, TODO: 问一下师兄或者大佬, 到底啥时候要写类, 啥时候一个函数就可以解决
        ...
        Q = Q.view(beh, N, args.head_num, int(d_h)).permute(2, 0, 1, 3)
        ...
        Z_list = [value for value in Z]  #[head_num, beh, beh, N, hidden_dim]
        Z = torch.cat(Z_list, dim=-1)  #[beh, N, hidden_dim]
        # Z = torch.transpose(Z, 0, 1)
        # self.weight_dict['w_self_attention_cat'] = torch.matmul() 
...
class HGNN(nn.Module):
    def __init__(self, userNum, itemNum):
        ...
        self.alpha, self.i_input_w, self.u_input_w, self.i_concatenation_w, self.u_concatenation_w = self.init_weight()
    ...
    def init_weight(self):
        ...
        return alpha, i_input_w, u_input_w, i_concatenation_w, u_concatenation_w
