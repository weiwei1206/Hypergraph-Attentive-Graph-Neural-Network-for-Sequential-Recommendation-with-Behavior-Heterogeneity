...
#——————————————————————————————————————————————————————————————————————————————————以下
        ...
        for t in range(0, self.time_number):
            if t==0:
                ...
                for i, beh in enumerate(self.behavior):
                    ...
                    # self.embedding_dict['times_user_embedding'][t][beh] = torch.matmul((self.embedding_dict['times_user_embedding'][t][beh] + user_z[i]) /2, self.weight_dict['w_self_attention_user'])
                    # self.embedding_dict['times_item_embedding'][t][beh] = torch.matmul((self.embedding_dict['times_item_embedding'][t][beh] + item_z[i]) /2, self.weight_dict['w_self_attention_item'])
    ...
    def multi_head_self_attention(self, trans_w, embedding_t_1, embedding_t):  
        ...
        Q = Q.reshape(beh, N, args.head_num, int(d_h)).permute(2, 0, 1, 3)  #[beh, N, head_num, d]  [head_num, beh, N, d]
        ...
        Z = torch.sum(Z, dim=2)  #[head_num, beh, N, d_h]
